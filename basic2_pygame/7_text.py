...
# 적 enemy 캐릭터
enemy = pygame.image.load("C:/Users/TA9/Documents/python/basic2_pygame/enemy.png")
enemy_size = enemy.get_rect().size # 이미지의 크기를 구해옴
enemy_width =  enemy_size[0] # 캐릭터의 가로 크기
enemy_height = enemy_size[1] # 캐릭터의 세로 크기
enemy_x_pos = (screen_width - enemy_width) / 2 # 화면 가로의 절반 크기에 해당하는 곳에 위치 (가로 위치)
enemy_y_pos = (screen_height - enemy_height) / 2 # 화면 세로크기 가장 아래에 해당하는 곳에 위치 (세로 위치)
...
# 시작 시간
start_ticks = pygame.time.get_ticks() # 현재 tick을 받아옴(ms 단위)
...
while running:
    ...
    # 타이머 집어넣기
    # 경과 시간 계산 (ms 단위 -> 1000으로 나눠서 초 단위로 표시)
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000

    # render(출력할 글자, True, 글자 색상), 화면에 출력할 내용을 렌더링
    timer = get_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255)) 
